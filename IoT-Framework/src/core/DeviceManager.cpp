// ============================================================================
// DeviceManager.cpp - ULTIMATE MICRO-OS V5.0 (EVENT-DRIVEN) - AUDITED
// ============================================================================
...
// - Публикации событий через новую шину (v5.0)
//
...
// - Добавлен метод publishDeviceEventInternal() для публикации через новую шину
// - Добавлен метод publishDeviceEventNew() (публичный)
// - Добавлены вызовы publishDeviceEventInternal() в ключевые методы
...
DeviceManager::DeviceManager() {
    ...
    Serial.println("[DEV_MNGR] Instance created (v5.0)");
}
...
// ============================================================================
void DeviceManager::publishDeviceEventInternal(const char* eventType, const char* deviceName,
                                               bool success) {
    ShEventData event;
...
void DeviceManager::publishDeviceEventNew(const char* eventType, const char* deviceName,
                                          bool success) {
    publishDeviceEventInternal(eventType, deviceName, success);
}
...
void DeviceManager::init() {
    ...
    if (!initEthernet()) {
        ...
        publishDeviceEventInternal("ETH_INIT", nullptr, false);
    } else {
        publishDeviceEventInternal("ETH_INIT", nullptr, true);
    }
...
void DeviceManager::start() {
    ...
    updateStatus();
    scanI2cBus();
    publishDeviceEventInternal("START", nullptr, true);
}
...
void DeviceManager::stop() {
    ...
    logMessage("Stopped");
    publishDeviceEventInternal("STOP", nullptr, true);
}
...
void DeviceManager::handleCommand(const ShEventData* data) {
    switch (data->command) {
        ...
        case 0x0103: { // INIT_ETH
            ...
            response.payloadLen = strlen(response.payload);
            postEvent(SH_EVENT_CMD_RESPONSE, &response);
            publishDeviceEventInternal("ETH_CMD", nullptr, result);
            break;
        }
...
void DeviceManager::getDiagnostics(ShEventData* data) const {
    ...
    snprintf(data->payload, sizeof(data->payload),
            "i2c:%zu,err:%lu,scan:%lu,drv:%zu,scans:%lu,fnd:%lu,eth:%s,mac:%s,events:%lu",
            _detectedI2cCount,
            _errors,
            _lastScanMs,
            _activeDrivers.size(),
...
// ============================================================================
// 9. ОТПРАВКА СОБЫТИЙ (ОРИГИНАЛЬНЫЕ МЕТОДЫ, БЕЗ ИЗМЕНЕНИЙ)
// ============================================================================
...
void DeviceManager::publishI2cScanComplete() {
    ...
    data.payloadLen = strlen(data.payload);
    postEvent(SH_EVENT_MODULE_TICK, &data);
    publishDeviceEventInternal("I2C_SCAN", nullptr, true);
}
...
void DeviceManager::publishDriverError(const char* driverName, const char* error) {
    ...
    data.payloadLen = strlen(data.payload);
    postEvent(SH_EVENT_MODULE_TICK, &data);
    publishDeviceEventInternal("DRIVER_ERROR", driverName, false);
}
...
void DeviceManager::publishEthEvent(int32_t eventId, bool success, const char* info) {
    ...
    data.payloadLen = strlen(data.payload);
    postEvent(SH_EVENT_MODULE_TICK, &data);
    publishDeviceEventInternal("ETH_EVENT", info, success);
}

// ============================================================================
// 10. ИНИЦИАЛИЗАЦИЯ I2C (ВАША, БЕЗ ИЗМЕНЕНИЙ)
// ============================================================================
...
// ============================================================================
// 11. РЕГИСТРАЦИЯ ДРАЙВЕРОВ (ВАША, БЕЗ ИЗМЕНЕНИЙ)
// ============================================================================
bool DeviceManager::registerDriver(IDeviceDriver* driver) {
    ...
    if (!driver->init()) {
        ...
        publishDeviceEventInternal("DRIVER_INIT", driver->getDriverName(), false);
        return false;
    }
    ...
    publishDeviceEventInternal("DRIVER_REGISTER", driver->getDriverName(), true);
...
void DeviceManager::unregisterDriver(const char* driverName) {
    ...
    for (auto it = _activeDrivers.begin(); it != _activeDrivers.end(); ) {
        if (strcmp((*it)->getDriverName(), driverName) == 0) {
            ...
            publishDeviceEventInternal("DRIVER_UNREGISTER", driverName, true);
...
void DeviceManager::unregisterDriver(uint32_t driverId) {
    ...
    for (auto it = _activeDrivers.begin(); it != _activeDrivers.end(); ) {
        if ((*it)->getDriverId() == driverId) {
            ...
            publishDeviceEventInternal("DRIVER_UNREGISTER_ID", name, true);
...
void DeviceManager::clearDrivers() {
    ...
    publishDeviceEventInternal("CLEAR_DRIVERS", nullptr, true);
    updateStatus();
}
...
// ============================================================================
// 12. I2C СКАНИРОВАНИЕ (ВАШЕ, БЕЗ ИЗМЕНЕНИЙ)
// ============================================================================
...
bool DeviceManager::initEthernet() {
    ...
    publishDeviceEventInternal("ETH_INIT", _ethMac, true);
...
// ============================================================================
// 14. МАНИФЕСТ ЖЕЛЕЗА (ВАШ, БЕЗ ИЗМЕНЕНИЙ)
// ============================================================================
String DeviceManager::getHardwareManifest() const {
    ...
    doc["max_drivers"] = (int)MAX_DRIVERS_SLOTS;

// ============================================================================
// AudioManager.cpp - ULTIMATE MICRO-OS V5.0 (FULLY INTEGRATED)
// ============================================================================
...
// - Добавлена публикация события EVENT_AUDIO_TRACK_START
// - Добавлена публикация события EVENT_AUDIO_TRACK_COMPLETE
...
AudioManager::AudioManager() {
    ...
    Serial.println("[AUDIO] Instance created (v5.0)");
}
...
void AudioManager::getDiagnostics(ShEventData* data) const {
    ...
    snprintf(data->payload, sizeof(data->payload),
            "state:%d,health:%d,played:%lu,err:%lu,wdt:%lu,queue:%zu,evt:%lu",
            (uint8_t)_playState,
...
// ============================================================================
void AudioManager::publishAudioEvent(uint16_t trackId, bool isStart) {
    ShEventData event;
    memset(&event, 0, sizeof(ShEventData));
    event.type = isStart ? EVENT_AUDIO_TRACK_START : EVENT_AUDIO_TRACK_COMPLETE;
    event.senderId = _moduleId;
    event.targetModuleId = 0xFF;

    event.payload.intData.val1 = trackId;
    event.payload.intData.val2 = millis();

    snprintf(event.payload.statusStr, sizeof(event.payload.statusStr),
            "track:%d,state:%s",
            trackId,
            isStart ? "start" : "complete");
...
void AudioManager::clearQueue() {
    ...
    logMessage("Cleared %zu tasks from queue", cleared);
...
void AudioManager::processQueue() {
    ...
    publishAudioEvent(nextTask.trackId, true);
...
void AudioManager::handleTrackComplete() {
    ...
    // НОВОЕ: публикация через новую шину
    publishAudioEvent(_currentTrackId, false);
...
// ============================================================================
// 21. ДИАГНОСТИКА (ИЗМЕНЕНО: добавлен _totalEventsPublished)
// ============================================================================

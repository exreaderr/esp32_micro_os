// ============================================================================
// WebServerManager.cpp - ULTIMATE MICRO-OS V4.2.2 (EVENT-DRIVEN) - AUDITED
// ============================================================================
...
WebServerManager::WebServerManager() : _server(WEB_SERVER_PORT) {
    ...
    Serial.println("[WEB] Instance created (v4.2.2)");
}
...
// ============================================================================
void WebServerManager::safeStrCopy(char* dest, size_t destSize, const char* src) {
...
void WebServerManager::stop() {
    ...
    if (xSemaphoreTakeRecursive(_webMutex, pdMS_TO_TICKS(MUTEX_TIMEOUT_MS)) == pdTRUE) {
        if (_serverStarted) {
            _server.stop();
            _serverStarted = false;
        }
...
void WebServerManager::tick() {
    ...
    uint32_t now = millis();
    if (_onStatsUpdate && (now - _stats.lastRequestTime > 60000)) {
...
const char* WebServerManager::getStatus() const {
    ...
    snprintf(statusBuffer, sizeof(statusBuffer),
            ...
            state, _port, _stats.totalRequests, _stats.apiCalls, _stats.activeSessions);
    return statusBuffer;
}
...
void WebServerManager::getDiagnostics(ShEventData* data) const {
    ...
    snprintf(data->payload, sizeof(data->payload),
            "running:%d,req:%lu,auth:%lu,api:%lu,err:%lu,sessions:%lu",
            _serverStarted ? 1 : 0,
...
void WebServerManager::handleCommand(const ShEventData* data) {
    switch (data->command) {
        ...
        case 0x0803: { // GET_API_DOCS
            ...
            safeStrCopy(response.payload, sizeof(response.payload), docs.c_str());
            response.payloadLen = docs.length();  // <-- ИСПРАВЛЕНО!
            postEvent(SH_EVENT_CMD_RESPONSE, &response);
            break;
        }
        ...
        case 0x0805: { // GET_CSRF_TOKEN
            ...
            safeStrCopy(response.payload, sizeof(response.payload), token.c_str());
...
void WebServerManager::publishRequestEvent(const char* uri, const char* method,
    ...
    memset(&event, 0, sizeof(WebRequestEvent));
    safeStrCopy(event.uri, sizeof(event.uri), uri ? uri : "/");
    safeStrCopy(event.method, sizeof(event.method), method ? method : "GET");
...
void WebServerManager::publishAuthEvent(const char* username, const char* role, bool success) {
    WebAuthEvent event;
    memset(&event, 0, sizeof(WebAuthEvent));
    safeStrCopy(event.username, sizeof(event.username), username ? username : "unknown");
    safeStrCopy(event.role, sizeof(event.role), role ? role : "none");
    safeStrCopy(event.token, sizeof(event.token), _currentSessionToken.c_str());
...
void WebServerManager::publishSessionEvent(bool created) {
    ...
    safeStrCopy(data.payload, sizeof(data.payload),
               created ? "Session created" : "Session destroyed");
    data.payloadLen = strlen(data.payload);
    postEvent(SH_EVENT_MODULE_TICK, &data);
}
...
void WebServerManager::publishCsrfEvent(bool success) {
    ...
    data.value = success ? 1 : 0;
    safeStrCopy(data.payload, sizeof(data.payload),
               success ? "CSRF verified" : "CSRF failed");
    data.payloadLen = strlen(data.payload);
    postEvent(SH_EVENT_MODULE_TICK, &data);
}
...
void WebServerManager::begin(uint16_t port, const WebServerParams& params) {
    ...
    if (xSemaphoreTakeRecursive(_webMutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
        ...
        // Остановка сервера для переконфигурации
        if (_serverStarted) {
            _server.stop();
            _serverStarted = false;
        }

        // Инициализация системных эндпоинтов
        initSystemEndpoints();

        // Регистрация пользовательских эндпоинтов
        if (_onRegisterEndpoints != nullptr) {
            _onRegisterEndpoints(_server);
        }

        // Обработчик 404
        _server.onNotFound([this]() {  // <-- ИСПРАВЛЕНО!
...
void WebServerManager::registerApi(const char* uri, HTTPMethod method,
                                   JsonApiHandler handler, bool protectedEndpoint,
    ...
    if (xSemaphoreTakeRecursive(_webMutex, pdMS_TO_TICKS(200)) == pdTRUE) {
        ...
        if (_apiDocs.size() < _maxApiDocSlots) {
            ApiDoc doc;
            memset(&doc, 0, sizeof(ApiDoc));
            safeStrCopy(doc.uri, sizeof(doc.uri), uri);
            safeStrCopy(doc.method, sizeof(doc.method), methodToString(method));
            safeStrCopy(doc.description, sizeof(doc.description), description ? description : "");
        ...
        _server.on(uri, method, [this, handler, protectedEndpoint, uri]() {
            ...
            if (_params.enableCsrf && protectedEndpoint) {
                if (!verifyCsrfToken(_server.header(_csrfHeader).c_str())) {
                    _stats.csrfErrors++;
...
void WebServerManager::initSystemEndpoints() {
    ...
    registerApi("/api/auth", HTTP_POST, [this](JsonVariant req, JsonObject resp) {
        ...
        if (strlen(inputHash) > 0) {
            if (safeStringCompare(inputHash, _params.adminPasswordHash) ||
        ...
        if (!authSuccess && strlen(cardId) > 0) {
            if (safeStringCompare(cardId, _params.hostname)) {
                authSuccess = true;
            }
        }
        ...
        if (authSuccess) {
            ...
            safeStrCopy(info.token, sizeof(info.token), tokenBuf);
            safeStrCopy(info.role, sizeof(info.role), "admin");
            safeStrCopy(info.username, sizeof(info.username), "Administrator");
            safeStrCopy(info.cardId, sizeof(info.cardId), cardId);
...
bool WebServerManager::checkAccess(bool adminRequired) {
    ...
    if (!safeStringCompare(token.c_str(), _currentSessionToken.c_str())) {
...
bool WebServerManager::login(const char* passwordHash) {
    ...
    if (xSemaphoreTakeRecursive(_webMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        // <-- ИСПРАВЛЕНО!
        if (safeStringCompare(passwordHash, _params.adminPasswordHash) ||
            _params.adminPasswordHash[0] == '\0') {
            char tokenBuf[64];
            generateSecureToken(tokenBuf, sizeof(tokenBuf));
...
void WebServerManager::logout() {  // <-- НОВАЯ РЕАЛИЗАЦИЯ!
    clearSession();
...
bool WebServerManager::verifyCsrfToken(const char* token) {
    ...
    // Проверка TTL
    if (millis() - _csrfTokenCreated > WEB_CSRF_TOKEN_TTL) {
        generateCsrfToken();
        return false;
    }

    bool valid = safeStringCompare(token, _csrfToken.c_str());
    if (valid) {
        publishCsrfEvent(true);
    } else {
        publishCsrfEvent(false);
...
bool WebServerManager::checkDdosProtection() {
    ...
    if (clientIp == _lastClientIp) {
        ...
        if (_requestCounter > _params.ddosLimit) {
            if (now - _lastRequestMs < WEB_DDOS_WINDOW_MS) {
                _stats.ddosBlocks++;
                logMessage("DDoS block: %u", clientIp);
...
void WebServerManager::addCorsHeaders() {
    if (_params.corsEnabled) {
        ...
        _server.sendHeader("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE, OPTIONS, PATCH");
        _server.sendHeader("Access-Control-Allow-Headers", "Content-Type, Authorization, X-CSRF-Token");
        _server.sendHeader("Access-Control-Allow-Credentials", "true");
...
void WebServerManager::addCacheHeaders(const String& path) {
    if (_params.enableCache) {
        if (path.endsWith(".js") || path.endsWith(".css") ||
            ...
            path.endsWith(".jpeg") || path.endsWith(".woff") ||
...
// ============================================================================
bool WebServerManager::safeStringCompare(const char* a, const char* b) {
...
void WebServerManager::updateStats(const char* endpoint, bool authenticated, bool isApi) {
    ...
    if (endpoint != nullptr) {
        safeStrCopy(_stats.lastEndpoint, sizeof(_stats.lastEndpoint), endpoint);
    }
}
...
const char* WebServerManager::getContentType(const String& path) const {
    ...
    if (path.endsWith(".jpg") || path.endsWith(".jpeg")) return "image/jpeg";  // <-- ИСПРАВЛЕНО!
...
void WebServerManager::serializeStats(Stream& stream) const {
    ...
    doc["ddos_blocks"] = _stats.ddosBlocks;
...
void WebServerManager::streamDiagnosticInfo(Stream& stream) const {
    ...
    stream.printf(" CSRF Enabled: %s\n", _params.enableCsrf ? "YES" : "NO");
    stream.printf(" Cache Enabled: %s\n", _params.enableCache ? "YES" : "NO");
...
void WebServerManager::printStats() const {

// ============================================================================
// PazManager.cpp - ULTIMATE MICRO-OS V4.2.2 (EVENT-DRIVEN) - AUDITED
// ============================================================================
...
// - ИСПРАВЛЕНА инициализация I2C
// - ИСПРАВЛЕНА работа с температурой (temperatureRead)
...
PazManager::PazManager() {
    ...
    Serial.println("[PAZ] Instance created (v4.2.2)");
}
...
void PazManager::tick() {
    ...
    // Если паника активна - удерживаем актуатор в безопасном состоянии
    if (_panicTripped) {
        if (_actuatorPin != 255) {
...
void PazManager::getDiagnostics(ShEventData* data) const {
    ...
    snprintf(data->payload, sizeof(data->payload),
            "status:%d,panic:%d,net:%d,code:%d,count:%lu",
...
void PazManager::begin(uint8_t sdaPin, uint8_t sclPin, uint32_t wdtTimeoutMs) {
    ...
    if (_sdaPin == 0 || _sclPin == 0) {
        ...
        Wire.begin(_sdaPin, _sclPin);           // <-- ИСПРАВЛЕНО!
...
// ============================================================================
// 11. КОНТРОЛЬ ТЕМПЕРАТУРЫ (ИСПРАВЛЕНО)
// ============================================================================
void PazManager::checkCpuTemperature() {
    // Используем встроенную функцию temperatureRead()
    ...
    // Уровень 1: Норма (до warningThreshold)
...
void PazManager::notifySensorUpdated() {
    // Обновление датчика - сбрасываем таймер залипания
...
// ============================================================================
// 19. ПИНГ КОЛЛБЭК (ISR-БЕЗОПАСНЫЙ!)
// ============================================================================

...
public:
	...
	std::string Topic();
	std::string Payload();
	// ПАТЧ МикроОС #3 (M2): retain-флаг исходного PUBLISH — мосту нужно
	// транслировать retained-состояния устройств наверх «как есть».
...
private:
	...
	std::string _topic;
	std::string _payload;
	bool _retain = false;   // ПАТЧ МикроОС #3
};

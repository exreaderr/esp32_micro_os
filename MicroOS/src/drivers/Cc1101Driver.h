...
public:
    ...
    /// MARCSTATE (статус 0x35, чтение через burst-бит): состояние
    /// конечного автомата радио (RX=0x0D). Для бенча probe=3 (5.8.4-pre5).
    /// NB: адрес совпадает со стробом STX — ЧТЕНИЕ легально, STX под запретом.

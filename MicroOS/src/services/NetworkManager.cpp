...
void NetworkService::registerExtensions() {
    ConfigService::getInstance().addFields("Сеть", {
        ...
        // 5.9.0: ЧЕЛОВЕЧЕСКОЕ имя для инфо-карточки/шапки (решение
        // владельца 09.09.2026 — «поименовать флот»: smart_light'ов будет
        // несколько, каждому своё имя). Русскоязычное, НЕ hostname:
        // hostname — технический реквизит сети, sys.name — для людей.

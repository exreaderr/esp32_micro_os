...
private:
    ...
    // 5.9.4 (дополнение 3 ветки weather_gate 13.09): замер ЗАНЯТЫХ сокетов
    // lwIP из sketch — открываем UDP-сокеты до отказа (cap 24), считаем,
    // закрываем. Печать раз в час: нарастание «~1 сокет/час» будет видно
    // задолго до клинча, а пары «час X → N» дадут скорость утечки.
    uint8_t  socketsInUse() const;
    uint32_t _lastSockReportMs = 0;   // последний часовой отчёт

...
#ifndef SMQTT_WIRED_ONLY
	// (Патч МикроОС M1: в проводном контуре этот блок не компилируется —
	// ссылка на WiFiClass утянула бы в прошивку весь WiFi-стек, +337 КБ.)
...
void sMQTTBroker::publish(sMQTTClient *client, sMQTTTopic *topic, sMQTTMessage *msg)
{
	...
	event.setRetain(msg->isRetained());   // ПАТЧ МикроОС #3 (M2)
	onEvent(&event);

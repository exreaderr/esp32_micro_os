...
void registerSmartLockConfig() {
    ...
    // === ГРУППА: Индикация (0.5.17, поля добавлены В КОНЕЦ схемы) ===
    // Статус-лента WS2812 (решение владельца 11.09.2026 — «как у шлюза»).
    // Пин из конфига: GPIO12 — единственный свободный выход WT32-ETH01
    // (страппинг MTDI: DIN ленты высокоомный, на бут не влияет; в манифест
    // не объявляем — conformance отвергает 0/12).
    cfg.addFields("Индикация", {
        { "led.enabled", ConfigType::BOOL, "false", 0, 0, CFG_NONE,
          "Индикация", "Статус-лента WS2812 включена" },
        { "led.pin",     ConfigType::UINT, "12", 0, 39, CFG_NONE,
          "Индикация", "GPIO данных ленты (свободен только 12)" },
        { "led.count",   ConfigType::UINT, "3", 1, 16, CFG_NONE,
          "Индикация", "Светодиодов в ленте (0=замок,1=кнопка,2=ПАЗ)" },
    });
}

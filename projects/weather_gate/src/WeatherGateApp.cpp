...
void WeatherGateApp::registerExtensions() {
    ...
    ok = ConfigService::getInstance().addFields("Телеметрия и сторожа", {
        ...
        // 0.7.2 (ядро 5.9.5 «сторож тишины»): регистрация поля по просьбе
        // ядерной ветки — драйвер читает cfgGetInt("rf.silence_min", 15)
        // и без регистрации, здесь поле становится видимым и правится из
        // /admin. 0 = сторож «RX, но тишина» выключен, потолок 240 мин.
        // CFG_NONE: драйвер читает значение при каждом обходе, ребут
        // не нужен.
        {"rf.silence_min",     ConfigType::UINT, "15", 0, 240,
         CFG_NONE, "Телеметрия и сторожа",
         "Сторож «RX, но тишина» CC1101, мин (0=выкл)"},
    });

...
//   LED статус-лента WS2812 (W6, 0.7.1): GPIO2 (страппинг: на время
//     UART-прошивки ленту отключать / держать LOW; одиночный LED не
//     распаян — к плате индикаторы не подключены, решение владельца 09.09)
//   Safe Mode:    GPIO15, кнопка на GND (активный низ)
//   Резерв:       GPIO12 (никогда не CS: HIGH при буте = нет загрузки)
//   ЭРРАТА ШЕЛКОГРАФИИ IO5/IO35 — ЗАКРЫТА 09.09.2026 (параллельное
//   расследование с ядерной веткой): правильный ответ GPIO35 (input-only,
//   уже занят под MISO). Ноги GPIO5 на гребёнке НЕТ — в резерв не брать.
...
struct WeatherGatePins {
    ...
    uint8_t cc1101Miso = 35;   // input-only; эррата шелка IO5/IO35 закрыта 09.09: это GPIO35

// ============================================================================
// WgStatusLedModule.cpp — реализация статус-LED шлюза (W6, порт эталона M5)
// ============================================================================
#include "WgStatusLedModule.h"
#include <services/ConfigService.h>
#include <services/HealthMonitor.h>

WgStatusLedModule& WgStatusLedModule::getInstance() {
    static WgStatusLedModule inst;
    return inst;
}
...
// ЖИЗНЕННЫЙ ЦИКЛ
// ============================================================================
void WgStatusLedModule::init() {
    if (!cfgGetBool("led.enabled", false)) {
        log(LogLevel::Info, "led.enabled=0 — статус-LED выключен (нет ленты?)");
        return;
    }
    _pin    = (uint8_t)cfgGetUInt("led.pin", 2);
    _count  = (uint8_t)cfgGetUInt("led.count", 3);
    ...
    // Маппинг пикселей: "wg.radio,wg.bme280,wg.storm" — по имени проверки.
    char mapStr[96];
    cfgGetStr("led.map", mapStr, sizeof(mapStr), "wg.radio,wg.bme280,wg.storm");
    _mapLen = 0;
    char* p = mapStr;
...
void WgStatusLedModule::start() {
    if (_hwReady) {
        render();
        _started = true;
        log(LogLevel::Info, "WgStatusLedModule started");
    }
}

void WgStatusLedModule::stop() {
    if (_strip != nullptr) {
        _strip->clear();
        _strip->show();
    }
...
void WgStatusLedModule::tick() {
    if (!_hwReady) return;
    // Пульс «нет данных» — раз в тик фаза; рендер по тику (500 мс),
...
// РЕНДЕР
// ============================================================================
int8_t WgStatusLedModule::checkIndexByName(const char* name) const {
